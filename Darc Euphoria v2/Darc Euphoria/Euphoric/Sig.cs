...
namespace Darc_Euphoria.Euphoric
{
    internal static class Sig
    {
        ...
        private static void Dump(IntPtr module, int moduleSize)
        {
            _dump = Memory.ReadBytes((int) module, moduleSize);
        }

        private static bool CheckSig(int index, Signature signature)
        {
            for (var i = 0; i < signature.ByteArray.Length; i++)
            {
                if (signature.Mask[i] == '?')
                    continue;
        ...
        public static int COffset(string sig, int offset = 0, int extra = 0)
        {
            return GetOffset(sig, offset, extra, (IntPtr) Memory.Client, Memory.client_size);
        }

        public static int EOffset(string sig, int offset = 0, int extra = 0)
        {
            return GetOffset(sig, offset, extra, (IntPtr) Memory.Engine, Memory.engine_size);
        }

        public static int GetOffset(string sig, int offset, int extra, IntPtr module, int moduleSize)
        {
            Dump(module, moduleSize);

            var signature = new Signature(sig, offset);
            ...
            for (var i = 0; i < moduleSize; i++)
                if (signature.Address == IntPtr.Zero && CheckSig(i, signature))
                {
                    ...
                    if (signature.Address != IntPtr.Zero)
                        return BitConverter.ToInt32(Memory.ReadBytes((int) signature.Address, 4), 0) + extra -
                               module.ToInt32();
                }
        ...
        public static Signature GetSignature(Signature sig, IntPtr module, int moduleSize)
        {
            var dumped = Memory.ReadBytes((int) module, moduleSize);
            ...
            for (var i = 0; i < moduleSize; i++)
                if (sig.Address == IntPtr.Zero && CheckSig(i, sig))
                {
                    ...
                    if (sig.Address != IntPtr.Zero) return sig;
                }
        ...
        public static int FindPattern(byte[] pattern, string mask, int moduleBase, int moduleSize)
        {
            ...
            if (WinAPI.ReadProcessMemory(Memory.pHandle, (IntPtr) moduleBase, moduleBytes, (uint) moduleSize,
                out numBytes))
                for (var i = 0; i < moduleSize; i++)
                {
                    var found = true;
        ...
        public static void InitOffsets()
        {
            ...
            Offsets.dwGameRulesProxy =
                COffset("A1 ? ? ? ? 85 C0 0F 84 ? ? ? ? 80 B8 ? ? ? ? ? 0F 84 ? ? ? ? 0F 10 05", 0x1);
            ...
            //Offsets.dw_RevealRankFn = COffset("55 8B EC 8B 0D ? ? ? ? 85 C9 75 ? A1 ? ? ? ? 68 ? ? ? ? 8B 08 8B 01 FF 50 ? 85 C0 74 ? 8B C8 E8 ? ? ? ? 8B C8 EB ? 33 C9 89 0D ? ? ? ? 8B 45 ? FF 70 ? E8 ? ? ? ? B0 ? 5D");
            Offsets.dw_RevealRankFn = COffset("C7 00 ? ? ? ? E8 ? ? ? ? 83 EC 08 8D 4E 74", 2);

            Offsets.LastOutGoingCommand = Memory.Engine +
                                          EOffset(
            ...
            Offsets.dwClientState_GetLocalPlayer =
                Convert.ToInt32(EOffset("8B 80 ? ? ? ? 40 C3", 0x2).ToString("X").Substring(4), 16);
            Offsets.dwClientState_IsHLTV =
                Convert.ToInt32(EOffset("80 BF ? ? ? ? ? 0F 84 ? ? ? ? 32 DB", 0x2).ToString("X").Substring(4), 16);
            Offsets.dwClientState_Map =
                Convert.ToInt32(EOffset("05 ? ? ? ? C3 CC CC CC CC CC CC CC A1", 0x1).ToString("X").Substring(4), 16);
            Offsets.dwClientState_MapDirectory =
                Convert.ToInt32(EOffset("05 ? ? ? ? C3 CC CC CC CC CC CC CC 80 3D", 0x1).ToString("X").Substring(4),
                    16);
            Offsets.dwClientState_MaxPlayer =
                Convert.ToInt32(
                    EOffset("A1 ? ? ? ? 8B 80 ? ? ? ? C3 CC CC CC CC 55 8B EC 8A 45 08", 0x7).ToString("X")
                        .Substring(4), 16);
            Offsets.dwClientState_PlayerInfo =
                Convert.ToInt32(EOffset("8B 89 ? ? ? ? 85 C9 0F 84 ? ? ? ? 8B 01", 0x2).ToString("X").Substring(4), 16);
            Offsets.dwClientState_State =
                Convert.ToInt32(EOffset("83 B8 ? ? ? ? ? 0F 94 C0 C3", 0x2).ToString("X").Substring(4), 16);
            Offsets.dwClientState_ViewAngles =
                Convert.ToInt32(EOffset("F3 0F 11 80 ? ? ? ? D9 46 04 D9 05", 0x4).ToString("X").Substring(4), 16);

            Offsets.dwGameDir = EOffset("68 ? ? ? ? 8D 85 ? ? ? ? 50 68 ? ? ? ? 68", 0x1);
            Offsets.dwGlobalVars = EOffset("68 ? ? ? ? 68 ? ? ? ? FF 50 08 85 C0", 0x1);
            Offsets.dwSetClanTag = FindPattern(new byte[] {0x53, 0x56, 0x57, 0x8B, 0xDA, 0x8B, 0xF9, 0xFF, 0x15},
                "xxxxxxxxx", Memory.Engine, Memory.engine_size);
            Offsets.dw_SetConVar = FindPattern(new byte[] {0x8D, 0x4C, 0x24, 0x1C, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x56},
                "xxxxx????x", Memory.Engine, Memory.engine_size);
            ...
            Offsets.dw_clientCmd = FindPattern(
                new byte[]
                {
                    ...
                    0x50, 0x34, 0x50, 0xA1
                }, "xxxxx????xx????xxx????x????xxxxxxxxx", Memory.Engine, Memory.engine_size);

            Offsets.dwSendPackets =
                (int) GetSignature(new Signature("B3 01 8B 01 8B 40 10 FF D0 84 C0 74 0F 80 BF ? ? ? ? ? 0F 84", 0x1),
                    (IntPtr) Memory.Engine, Memory.engine_size).Address - Memory.Engine;
        ...
        public static void InitNetvars(Dictionary<string, Dictionary<string, int>> _tables)
        {
            ...
            Netvars.m_fAccuracyPenalty = _tables["DT_WeaponCSBase"]["m_fAccuracyPenalty"];
        }
    }
}